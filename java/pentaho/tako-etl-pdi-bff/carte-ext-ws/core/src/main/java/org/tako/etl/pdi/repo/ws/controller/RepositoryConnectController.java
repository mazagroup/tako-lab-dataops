...
package org.tako.etl.pdi.repo.ws.controller;
...
import org.tako.etl.pdi.repo.ws.IConnectedRepositoryInstance;
import org.tako.etl.pdi.repo.ws.adapters.UIRepositoryDirectoryAdapter;
import org.tako.etl.pdi.repo.ws.model.BrowseModel;
import org.tako.etl.pdi.repo.ws.model.RepositoryModel;
import org.tako.etl.pdi.repo.ws.timeout.RepositorySessionTimeoutHandler;
...
public class RepositoryConnectController implements IConnectedRepositoryInstance {
	...
	private static LogChannelInterface log = KettleLogStore.getLogChannelInterfaceFactory()
			.create(RepositoryConnectController.class);
	...
	/*
	 * public void setParentShell( Shell shell ) { this.parentShell = shell; }
	 * 
	 * public Shell getParentShell( ) { return this.parentShell; }
	 */
	public boolean help() {
		/*
		 * spoonSupplier.get().getShell().getDisplay().asyncExec( () ->
		 * HelpUtils.openHelpDialog( this.parentShell, BaseMessages.getString( PKG,
		 * "RepositoryDialog.Dialog.Tile" ), HELP_URL, BaseMessages.getString( PKG,
		 * "RepositoryDialog.Dialog.Header" ) ) );
		 */
		throw new UnsupportedOperationException("RepositoryConnectController.help()");
		// return true;
	}
	...
	public String createConnection() {
		...
		spoonSupplier.get().getShell().getDisplay().asyncExec(() -> {
			throw new UnsupportedOperationException("RepositoryConnectController.createConnection()");
			/*
			 * DatabaseDialog databaseDialog = new DatabaseDialog(
			 * spoonSupplier.get().getShell(), new DatabaseMeta() ); databaseDialog.open();
			 * DatabaseMeta databaseMeta = databaseDialog.getDatabaseMeta(); if (
			 * databaseMeta != null ) { if ( !isDatabaseWithNameExist( databaseMeta, true )
			 * ) { addDatabase( databaseMeta ); future.complete( databaseMeta.getName() ); }
			 * else { DatabaseDialog.showDatabaseExistsDialog(
			 * spoonSupplier.get().getShell(), databaseMeta ); } } future.complete( "None"
			 * );
			 */
		});
	...
	public String editDatabaseConnection(String database) {
		throw new UnsupportedOperationException("RepositoryConnectController.editDatabaseConnection()");
		/*
		 * CompletableFuture<String> future = new CompletableFuture<>();
		 * spoonSupplier.get().getShell().getDisplay().asyncExec( () -> { DatabaseMeta
		 * databaseMeta = getDatabase( database ); String originalName =
		 * databaseMeta.getName(); DatabaseDialog databaseDialog = new DatabaseDialog(
		 * spoonSupplier.get().getShell(), databaseMeta ); databaseDialog.open(); if (
		 * !isDatabaseWithNameExist( databaseMeta, false ) ) { save(); future.complete(
		 * databaseMeta.getName() ); } else { DatabaseDialog.showDatabaseExistsDialog(
		 * spoonSupplier.get().getShell(), databaseMeta ); databaseMeta.setName(
		 * originalName ); databaseMeta.setDisplayName( originalName ); future.complete(
		 * originalName ); } } ); JSONObject jsonObject = new JSONObject(); try {
		 * jsonObject.put( "name", future.get() ); return jsonObject.toJSONString(); }
		 * catch ( Exception e ) { jsonObject.put( "name", "None" ); return
	...
	public String browse() {
		throw new UnsupportedOperationException("RepositoryConnectController.browse()");
		/*
		 * Spoon spoon = spoonSupplier.get(); CompletableFuture<String> name = new
		 * CompletableFuture<>(); Runnable execute = () -> { DirectoryDialog
		 * directoryDialog = new DirectoryDialog( spoonSupplier.get().getShell() );
		 * name.complete( directoryDialog.open() ); }; if ( spoon.getShell() != null ) {
		 * spoon.getShell().getDisplay().asyncExec( execute ); } else { execute.run(); }
		 * try { return name.get(); } catch ( Exception e ) { return "/"; }
		 */
	}

	@SuppressWarnings("unchecked")
	public String browse(BrowseModel model) {
		try {
			if ( model.getTargetDirPath() != null) {
				return loadRepositoryObjects( model );
			}
			else {
				return loadRepositoryDirectoryTree(model.getRepo(), model.getUser(), model.getPass());
			}
	...
	private String loadRepositoryDirectoryTree(String repOption, String userOption, String passOption)
		...
		return repoDirModel.toJSON().toJSONString();
	}
	
	private String loadRepositoryObjects( BrowseModel model )
			throws KettleException {
		final Repository repo = openRepository( model.getRepo(), model.getUser(), model.getPass() );
		final RepositoryDirectoryInterface dir = repo.findDirectory( model.getTargetDirPath() );

		final List<RepositoryElementMetaInterface> dirObjectList = loadRepositoryObjects( dir,true,true, repo );
		
		JSONArray list = new JSONArray();
		for ( RepositoryElementMetaInterface obj : dirObjectList ) {
			final JSONObject jObj = new JSONObject();
			jObj.put("name", obj.getName());
			jObj.put("objid", obj.getObjectId().toString());
			jObj.put("objtype", obj.getObjectType().toString());
			jObj.put("lastModBy", obj.getModifiedUser());
			jObj.put("lastModDate", obj.getModifiedDate().toString());
			jObj.put("repo", model.getRepo());
			list.add(jObj);
		}
		return list.toJSONString();
	}
	
	 private  List<RepositoryElementMetaInterface> loadRepositoryObjects( RepositoryDirectoryInterface dir,
	...
	public boolean updateRepository(String id, Map<String, Object> items) {
		...
		try {
			Repository repository = pluginRegistry.loadClass(RepositoryPluginType.class, repositoryMeta.getId(),
					Repository.class);
			repository.init(repositoryMeta);
	...
	public void reconnectToRepository(String username, String password) throws KettleException {
		...
		reconnectToRepository(currentRepository, (ReconnectableRepository) currentRepositoryInstance, username,
				password);
	}
